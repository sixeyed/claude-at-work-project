"""Running Messaging's Alembic migrations (Conventions §11).

The migration scripts live *inside* the `messaging` package rather than beside
it, so they are part of the wheel. The service image installs with
`--no-editable` and drops the source tree, and migrations that are not packaged
are migrations that cannot run in the container that needs them.
